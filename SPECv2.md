...
xar/
├── src/
│   ├── cli.ts                    # CLI 入口（commander，命令名 xar）
...
│   │   └── list.ts               # xar list
│   ├── daemon/
│   │   ├── index.ts              # Daemon 主入口（启动所有子系统）
│   │   ├── server.ts             # IPC Server（WebSocket over Unix socket）
│   │   ├── scheduler.ts          # 内置 cron（agent 内部定时任务）
│   │   └── pid.ts                # PID 文件管理
│   ├── agent/
│   │   ├── manager.ts            # Agent 生命周期（加载配置、注册、注销）
...
│   │   ├── memory.ts             # Memory 分层管理与压缩
│   │   └── deliver.ts            # 出站投递（通过 IPC → xgw）
│   ├── ipc/
│   │   ├── types.ts              # IPC 消息类型定义
│   │   ├── server.ts             # createIpcServer()
│   │   └── client.ts             # createIpcClient()（CLI 命令用）
│   └── types.ts                  # 共享类型定义
├── vitest/
│   ├── unit/
│   └── pbt/
├── package.json
├── tsconfig.json
├── tsup.config.ts                # 单 entry: src/cli.ts，带 shebang
├── vitest.config.ts
├── SPECv2.md
└── USAGE.md
```
...
`<thread_id_slug>`：thread 目录绝对路径经 slug 化处理（非字母数字替换为 `-`，超长时截断加哈希后缀），用于 session 文件和 memory 文件命名。

---

## Agent 配置格式

`~/.theclaw/agents/<id>/config.json`：
...
4. 将 agent 状态写入注册表（`~/.theclaw/agents/<id>/config.json` 中的 `status: "stopped"`）。
5. 若 agent 已存在，报错退出（退出码 1）。

**`xar start <id>` 行为**：
1. 若 daemon 未运行，报错退出（退出码 1，提示先运行 `xar daemon start`）。
2. 通过 IPC 发送 `agent_start` 消息给 daemon。
3. Daemon 为该 agent 启动 run-loop（若尚未运行）。
4. 更新 agent 状态为 `started`。

**`xar stop <id>` 行为**：
1. 通过 IPC 发送 `agent_stop` 消息给 daemon。
2. Daemon 停止向该 agent 的队列投递新消息，等待当前消息处理完成后停止 run-loop。
3. 更新 agent 状态为 `stopped`。

**`xar status [<id>]`**：
- 无 `<id>`：列出所有 agent 的状态摘要。
- 有 `<id>`：显示单个 agent 的详细状态（状态、inbox 积压数、最近活动时间、当前处理消息数）。
- `--json`：输出 JSON。
...
每次 LLM 调用前，`context.ts` 估算当前 session token 数。超过 `session_compact_threshold_tokens` 时，先发起一次独立的 `pai.chat()` 调用做 session 内压缩，结果写入 `memory/thread-<slug>.md`，然后再构建本次调用的 context。

#### 跨 session Memory（异步路径）

run-loop 处理完每条消息后，通过内部事件触发 memory processor（独立 async 任务）：

```
run-loop → emit('session_turn_completed', { agentId, threadId, peerId, newMessages })
                          ↓ 异步（不阻塞 run-loop）
              memory processor
                → 估算 peer memory token 数
                → 超过阈值时：pai.chat() 做摘要 → 写 memory/user-<peer_id>.md
                → 定期（cron）：pai.chat() 做 agent 级摘要 → 写 memory/agent.md
```

memory processor 失败不影响 run-loop，错误记录到 daemon 日志。
...
文件：`~/.theclaw/agents/<id>/logs/agent.log`

记录内容：
- 消息处理开始/完成（含 thread 路由结果）
- LLM 调用状态（耗时、token 用量、重试次数）
- Memory 更新触发/完成
- 出站投递状态
