...
describe('Router Determinism Property Tests', () => {
  ...
  const configArb = fc.record({
    ...
    kind: fc.oneof(fc.constant('system' as const), fc.constant('user' as const)),
    pai: fc.record({
      provider: fc.string({ minLength: 1 }),
      model: fc.string({ minLength: 1 }),
    }),
    routing: routingModeArb.map((mode) => ({
      mode,
  ...
  it('Property: Router Determinism - same inputs always produce same thread ID', () => {
    fc.assert(
      fc.property(configArb, idArb, (config, id) => {
        const source = `external:tui:default:dm:${id}:${id}`
  ...
  it('Property: Different peer IDs produce different thread IDs in reactive dm mode', () => {
    fc.assert(
  ...
  it('Property: reactive dm routes to peers/<id>, autonomous routes to conversations/<id>', () => {
    fc.assert(
      fc.property(idArb, (id) => {
        const source = `external:tui:default:dm:${id}:${id}`

        const reactiveConfig: AgentConfig = {
          agent_id: 'test', kind: 'user',
          pai: { provider: 'openai', model: 'gpt-4o' },
          routing: { mode: 'reactive', trigger: 'mention' },
          memory: { compact_threshold_tokens: 8000, session_compact_threshold_tokens: 4000 },
          retry: { max_attempts: 3 },
        ...
        const reactiveThread = determineThreadId(reactiveConfig, source)
        const autonomousThread = determineThreadId(autonomousConfig, source)

        return reactiveThread.startsWith('peers/') && autonomousThread.startsWith('conversations/')
      }),
      { numRuns: 100 },
    )
  })
