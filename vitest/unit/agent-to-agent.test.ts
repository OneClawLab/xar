...
describe('agent-to-agent interaction', () => {
  ...
  describe('auto-reply routing: worker replies back to sender', () => {
    it('delivers assistant reply to sender agent when source is internal', async () => {
      const { processTurn } = await import('../../src/agent/turn.js')
      vi.mocked(processTurn).mockClear()
      vi.mocked(processTurn).mockResolvedValueOnce({
        newMessages: [{ role: 'assistant', content: 'WORKER_REPLY' }],
      } as any)

      const deliveredMessages: Array<{ agentId: string; message: InboundMessage }> = []
      const sendToAgent = vi.fn((agentId: string, message: InboundMessage) => {
      ...
      queue.push({ source: 'internal:agent:conv-xyz:orchestrator', content: 'do work' })
      queue.close()
      await loop.start()

      expect(sendToAgent).toHaveBeenCalledOnce()
      expect(deliveredMessages[0]!.agentId).toBe('orchestrator')
      expect(deliveredMessages[0]!.message.content).toBe('WORKER_REPLY')
      // Reply source identifies worker as sender, preserves conv-id
      expect(deliveredMessages[0]!.message.source).toBe('internal:agent:conv-xyz:worker')
    })

    it('does NOT auto-reply for external source messages', async () => {
      const { processTurn } = await import('../../src/agent/turn.js')
      vi.mocked(processTurn).mockClear()
      vi.mocked(processTurn).mockResolvedValueOnce({
        newMessages: [{ role: 'assistant', content: 'hello user' }],
      } as any)

      const sendToAgent = vi.fn(() => true)
      const queue = new AsyncQueueImpl<InboundMessage>()
      const loop = new RunLoopImpl('agent-a', queue, new Map(), makePai(), undefined, sendToAgent)

      queue.push({ source: 'external:tui:main:dm:conv-1:user-1', content: 'hi' })
      queue.close()
      await loop.start()
    ...
    it('logs warning when sender agent is not running', async () => {
      const { processTurn } = await import('../../src/agent/turn.js')
      vi.mocked(processTurn).mockClear()
      vi.mocked(processTurn).mockResolvedValueOnce({
      ...
      const warnSpy = vi.fn()
      const logger = { info: vi.fn(), warn: warnSpy, error: vi.fn(), debug: vi.fn(), close: async () => {} }
      const sendToAgent = vi.fn(() => false) // sender not running

      const queue = new AsyncQueueImpl<InboundMessage>()
      const loop = new RunLoopImpl('worker', queue, new Map(), makePai(), logger, sendToAgent)

      queue.push({ source: 'internal:agent:conv-xyz:gone-agent', content: 'task' })
      queue.close()
      await loop.start()

      const warns = warnSpy.mock.calls.map((c) => String(c[0]))
      expect(warns.some((m) => m.includes('gone-agent') && m.includes('not running'))).toBe(true)
    })

    it('preserves conv-id in reply source for correct thread routing at orchestrator', async () => {
      const { processTurn } = await import('../../src/agent/turn.js')
      vi.mocked(processTurn).mockClear()
      vi.mocked(processTurn).mockResolvedValueOnce({
      ...
      const convId = 'my-special-conv'
      queue.push({ source: `internal:agent:${convId}:orchestrator`, content: 'task' })
      queue.close()
      await loop.start()

      const replySource = deliveredMessages[0]!.message.source
      const parsed = parseSource(replySource)
      // Orchestrator (per-peer routing) will route this to threads/peers/worker
      expect(parsed.kind).toBe('internal')
      expect(parsed.conversation_id).toBe(convId)
      expect(parsed.sender_agent_id).toBe('worker')
    })
  })
